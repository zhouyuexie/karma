## 概述
为了更好地为你服务，Karma 是通过一个配置文件知道你的项目的信息，然后再测试项目。生成一个初始的配置文件的最简单的方法是使用 `karma init` 。此页列出了所有可用的配置选项。

注意: 大多数 framework adapters, reporters, preprocessors and launchers 需要作为插件[plugins]加载进来.

Karma 的配置文件可以用JavaScript或CoffeeScript作为一个普通的Node.js模块加载。

除非提供参数，否则 Karma CLI 将会去这些配置文件里找：
...
在配置文件中配置代码，通过设置 `module.exports` 指向一个函数接受一个参数一起：配置对象。
